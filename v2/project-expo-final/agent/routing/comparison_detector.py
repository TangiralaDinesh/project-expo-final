...
class ComparisonQueryDetector:
    ...
    def _detect_regex(self, query: str) -> Optional[ComparisonDecision]:
        ...
        has_vs = any(kw in query_lower for kw in self.DECISION_KEYWORDS["vs"])
        has_comparison = any(
            kw in query_lower 
            for kws in self.DECISION_KEYWORDS.values() 
            for kw in kws
        )
        
        if not (has_vs or has_comparison):
            return None
    ...
    async def _detect_llm(self, query: str) -> ComparisonDecision:
        """Use LLM to detect comparison queries."""
