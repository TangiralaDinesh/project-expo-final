...
# Code/bash execution queries — needs direct computation, not retrieval
_COMPUTATION_PATTERN = re.compile(
    r"""
    (?:
        # Bash/shell commands
        (?:bash|shell|command|execute|run)\s+(?:command|script|code)
        |(?:ls|find|grep|cat|echo|cd|mkdir|rm|cp|mv|pwd|which|whereis)\s*\(
        |how\s+(?:many|often|many|do\s+i).*(?:file|directory|count|list)
        |list.*(?:file|directory|folder|content)
        |what.*(?:file|directory|folder).*(?:in|contain|is)
        |count.*(?:character|letter|occurrence|instance)
        |how\s+many\s+(?:r's|'s|a's|e's|l's).*(?:in|within)
        
        # Python/script execution
        |execute\s+(?:python|code|script)
        |run\s+(?:python|script|calculation)
        |calculate|compute|arithmetic|count
        |compare\s+\d+.*(?:vs|versus|with|and).*\d+
        
        # File system operations
        |file.*(?:list|content|read|show)
        |directory.*(?:list|content|show)
        |directory\s+(?:structure|tree)
        |folder.*(?:content|structure|list)
        
        # Math/validation queries (usually parametric but context-dependent)
...
def _regex_fast_path(query: str) -> Optional[GateDecision]:
    ...
    if intent == "COMPUTATION":
        return GateDecision(
            ...
            mode="COMPUTATION",
            reason="computation_regex",
            confidence=0.93,  # Regex match is very reliable for code execution
        )
...
- "mode": "PARAMETRIC" | "SEMANTIC" | "CODE" | "COMPUTATION" | "HYBRID" | "SKILL" | "URL_DIRECT"
...
- COMPUTATION: Needs actual code execution (bash commands, Python scripts, file operations, calculations).
...
IMPORTANT: If user wants to execute code, list files, count occurrences, or run calculations — use COMPUTATION mode."""
...
async def entry_gate(
    ...
    *,
    ...
    3. Computation detection (needs code execution)
    4. Regex patterns (math, definitions, current events)
