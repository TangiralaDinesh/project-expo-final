...
if TYPE_CHECKING:
    from .reasoning import ThinkingProfile
...
class SatisfactionTracker:
    ...
    NEW (Phase 1): Per-concept satisfaction tracking for multi-concept queries.
    Concepts are extracted from query and tracked independently.
    """
    total_queries: int = 0
    ...
    concepts_satisfaction: dict[str, float] = field(default_factory=dict)  # Per-concept satisfaction (0-1)
    _last_query: str = ""
    _last_was_correction: bool = False
    corrections: list = field(default_factory=list)  # Track all corrections for Tier 1
    ...
    def get_recent_corrections(
        ...
        """Get correction patterns weighted by recency and domain relevance (Tier 1).
    ...
    def apply_to_thinking_profile(
        ...
        """Directly modify a thinking profile based on correction history (Tier 1).
        
        This is the FEEDBACK LOOP - corrections improve next query's thinking.
        """
    ...
    def extract_concepts(self, query: str) -> list[str]:
        ...
        Examples:
          ...
          "CDSL pricing and features" → ["CDSL", "pricing", "features"]
        ...
        while i < len(words):
            ...
            # Skip stopwords and small words
            if word in stopwords or len(word) < 2:
                i += 1
                continue
            
            # Capitalize as concept
            concept = words[i].strip(".,!?").capitalize()
            concepts.append(concept)
            i += 1
        
        return list(dict.fromkeys(concepts))[:6]  # Dedupe, limit to 6
    ...
    async def evaluate_satisfaction(
        ...
        """Evaluate satisfaction for each concept based on retrieved learnings.
        
        Returns: {concept: satisfaction_score (0-1)}
        
        Satisfaction thresholds:
          < 0.3: Very thin retrieval (needs follow-up)
          0.3-0.6: Partial coverage (might need follow-up)
          0.6-0.8: Good coverage
          > 0.8: Excellent coverage (satisfied)
        
        Threshold for "needs follow-up": 0.7
        """
        if concepts is None:
        ...
        for concept in concepts:
            # Count learnings relevant to this concept
    ...
    def get_unsatisfied_concepts(self, threshold: float = 0.7) -> list[str]:
        """Return concepts with satisfaction below threshold.
        
        These are candidates for follow-up retrieval.
        """
        return [
            concept for concept, score in self.concepts_satisfaction.items()
            if score < threshold
        ]
