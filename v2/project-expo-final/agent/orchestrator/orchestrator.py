    ...
    *,
    ...
    if gate_mode == "SEMANTIC":
        try:
            ...
            logger.warning(f"Intent classification failed: {e}, falling back to defaults")
            intent_analysis = None
    ...
    if intent_analysis:
        ...
        # COMPARISON INTENT: Create parallel nodes for each entity
        if intent_analysis.intent == QueryIntent.COMPARISON and len(focus_areas) >= 2:
            nodes = []
    ...
    # ── FALLBACK: Original comparison detection (if intent analysis failed) ──
