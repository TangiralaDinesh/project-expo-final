...
async def decompose_task(
    ...
    *,
    ...
    Also detects comparison queries and creates parallel retrieval nodes for each entity.
    """
    ...
    # Check for comparison queries first (Phase 1)
    ...
    if gate_mode == "SEMANTIC":
        ...
        # If comparison detected, create parallel nodes for each entity
        if comparison_result and comparison_result.is_comparison and len(comparison_result.entities) > 1:
            nodes = []
            entity_names = []
            
            # Phase 1: Create one retriever node per entity (parallel, no dependencies)
            for i, entity in enumerate(comparison_result.entities):
                node_id = f"compare_entity_{i}_{entity.name.replace(' ', '_')}"
                # Create focused retrieval task for this entity
                entity_task = (
                    f"Retrieve detailed and comprehensive information about {entity.name}. "
                    f"In context: {task}\n\n"
                    f"Ensure you gather sufficient information to fairly compare this entity with others. "
                    f"Include key features, benefits, drawbacks, costs, and use cases."
                )
            ...
            # Phase 1: Add validation node that depends on all entity nodes
            # This node validates that both entities were equally explored
            entity_node_ids = [n.node_id for n in nodes]
            validation_node = TaskNode(
                node_id="compare_validation",
                subagent_type=SubagentType.RETRIEVER,
                task=(
                    f"Validate comparison coverage for: {task}\n"
                    f"Entities to validate: {', '.join(entity_names)}\n"
                    f"Check: Were both entities thoroughly explored? "
                    f"If not, identify missing information and retrieve it."
                ),
                depends_on=entity_node_ids,
            )
            nodes.append(validation_node)
            
            # Phase 1: Add synthesis node that depends on validation
            synthesis_node = TaskNode(
                node_id="compare_synthesis",
                subagent_type=SubagentType.RETRIEVER,
                task=(
                    f"Compare and synthesize comprehensive analysis: {task}\n"
                    f"Entities compared: {', '.join(entity_names)}\n"
                    f"Provide balanced, detailed comparison covering all dimensions: "
                    f"features, costs, benefits, risks, use cases, and recommendation factors."
                ),
                depends_on=["compare_validation"],
            )
            nodes.append(synthesis_node)
            
            logger.info(f"Detected comparison query with entities: {entity_names} | "
    ...
    # Non-comparison SEMANTIC or HYBRID
    if gate_mode == "SEMANTIC":
        return Decomposition(nodes=[
...
async def run_orchestrator(
    ...
    *,
    ...
    logger.info(
        ...
        f"   Comparison query: {'Yes' if any('compare' in r.node_id for r in results) else 'No'}\n"
