...
async def should_use_graph(query: str, client: Optional[NIMClient] = None) -> bool:
    """Decide if a query benefits from graph traversal.
    Uses async LLM call to evaluate if the query is a relationship/multi-hop query.
    """
    ...
    try:
        ...
        logger.debug(f"should_use_graph error: {e}")
        return False
...
async def graph_enhanced_retrieval(
    ...
    *,
    ...
    # ── 2. Graph traversal ──
    if not await should_use_graph(query, client) or graph.entity_count == 0:
