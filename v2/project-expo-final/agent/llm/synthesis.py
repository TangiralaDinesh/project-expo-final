...
def _calculate_adaptive_max_tokens(
    ...
    - Simple query with few learnings: 2048
    - Medium query with moderate learnings: 4000-6000
    - Complex query with many learnings: 6000-8000
    - Very complex queries: up to 12000 for comprehensive answers
    ...
    # Remove the artificial low-output floor. The model should be allowed to
    # produce a full answer without stopping mid-sentence just because the
    # previous code enforced a small token budget.
...
async def global_synthesis_llm(
    ...
    *,
    ...
    if _detect_truncation(response):
        logger.warning(f"Response truncated at {adaptive_tokens} tokens, retrying with the maximum response budget")
        response = await client.chat(
            messages,
            temperature=0.2,
            max_tokens=16384,
