...
def _calculate_adaptive_max_tokens(
    ...
    """Dynamically calculate max_tokens based on query and learnings complexity (Phase 5).
    
    NOTE: This function is LEGACY and now returns 0 (unlimited) to match v1 behavior.
    The model is allowed to generate as much as it needs without artificial cutoffs.
    
    Streaming handles token management implicitly. Synthesis never gets truncated mid-sentence.
    """
    # Return 0 = unlimited tokens (match v1 behavior)
    # Let the LLM streaming handle token limits naturally
    return 0  # Unlimited - model generates naturally
...
async def global_synthesis_llm(
    ...
    *,
    ...
    # NOTE: adaptive_tokens now returns 0 (unlimited) for v1 behavior
    if adaptive_tokens > 0:
        logger.info(f"Synthesis using adaptive max_tokens: {adaptive_tokens}")
    else:
        logger.info("Synthesis using unlimited tokens (streaming mode)")
    
    messages = _build_synthesis_prompt(query, learnings, prompt_specificity)
    
    # Use unlimited tokens (v1 behavior - let model generate naturally)
    response = await client.chat(
        messages,
        temperature=0.2,
        max_tokens=None,  # Unlimited - model generates freely
    )
...
async def global_synthesis_llm_stream(
    ...
    *,
    ...
    async for delta in client.chat_stream(
        ...
        temperature=0.2,
        max_tokens=None,  # Unlimited - streaming handles naturally
    ):
        yield delta
...
async def direct_answer_llm(
    ...
    *,
    ...
    return await client.chat(
        ...
        temperature=0.3,
        max_tokens=None,  # Unlimited - no token cap
    )
