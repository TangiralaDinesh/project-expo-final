...
def _calculate_adaptive_max_tokens(
    ...
    This prevents truncation by allocating output tokens based on input complexity.
    """
    ...
    # Base calculation: allocate output tokens inversely to input size
    # More input context → reserve more tokens for comprehensive output
    if total_context_tokens < 1000:
        base_tokens = 1024  # Simple query, short learnings
    elif total_context_tokens < 2000:
        base_tokens = 1536  # Small to medium
    elif total_context_tokens < 4000:
        base_tokens = 2048  # Medium complexity
    elif total_context_tokens < 6000:
        base_tokens = 2560  # Higher complexity
    elif total_context_tokens < 8000:
        base_tokens = 3000  # Very high complexity
    else:
        base_tokens = 3500  # Extremely high complexity (Phase 5 enhancement)
    
    # Adjust based on query type
    if prompt_specificity == "expert":
        # Expert technical queries need more tokens for detailed explanations
        base_tokens = min(4096, base_tokens + 512)
    elif prompt_specificity == "casual":
        # Casual queries can be concise
        base_tokens = max(512, base_tokens - 256)
    
    # Ensure we never exceed reasonable limits
    return min(4096, base_tokens)
