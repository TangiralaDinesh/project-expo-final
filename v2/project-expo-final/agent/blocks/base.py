...
async def run_retriever_subagent(sub_input: SubagentInput) -> SubagentResult:
    ...
    try:
        ...
        logger.error(f"Semantic retriever block failed ({type(exc).__name__}): {exc}")
        return SubagentResult(
            subagent_type=SubagentType.RETRIEVER,
            success=False,
            error_reason=f"retriever_exception: {type(exc).__name__}: {exc}",
...
async def run_code_executor_subagent(sub_input: SubagentInput) -> SubagentResult:
    """Adapts the code execution block for direct computation tasks.
    
    Takes a task (query), generates and executes Python code to answer it,
    returns execution results as learnings.
    """
    from ..core.code_execution_integration import execute_analysis_code
    
    try:
        # Execute analysis code for this task
        result = await execute_analysis_code(
            query=sub_input.task,
            learnings=[],  # No prior learnings for initial execution
            sandbox_dir=sub_input.payload.get("sandbox_dir"),
        )
        ...
        if result is None:
            return SubagentResult(
                ...
                success=False,
                error_reason="code_execution_returned_none",
                session_id=sub_input.session_id,
                turn_id=sub_input.turn_id,
                parent_id=sub_input.parent_id,
            )
        
        # Convert execution result to learnings
        learnings = []
        if result.stdout:
            learnings.append(Learning(text=result.stdout))
        if result.reasoning:
            learnings.append(Learning(text=f"Execution reasoning: {result.reasoning}"))
        
        # Add file creation info if any
        for file_path in (result.files_created or []):
            learnings.append(Learning(text=f"Generated file: {file_path}"))
        ...
        return SubagentResult(
            ...
            learnings=learnings,
            source_urls=[],
            error_reason=result.stderr if result.stderr else "",
            session_id=sub_input.session_id,
            turn_id=sub_input.turn_id,
            parent_id=sub_input.parent_id,
        )
        ...
        return SubagentResult(
            subagent_type=SubagentType.CODE_GEN_EXECUTOR,
            success=False,
            error_reason=f"code_executor_exception: {exc}",
            session_id=sub_input.session_id,
            turn_id=sub_input.turn_id,
            parent_id=sub_input.parent_id,
        )
