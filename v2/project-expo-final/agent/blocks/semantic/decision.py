...
1. SUFFICIENT: Is the retrieved information enough to answer this sub-goal?
   ...
   - IMPORTANT: If this is a COMPARISON query (e.g., "X vs Y", "should I buy X or Y"):
     * DO NOT mark sufficient until BOTH entities have been explored with equal depth
     * If you only have info on X but not Y, or much more on X than Y, mark as insufficient
     * Comparison queries REQUIRE balanced exploration of all entities
...
2. NEXT_QUERIES: If not sufficient, generate 2-3 follow-up queries that would
   ...
   - If one entity has 50 learnings and another has 10, prioritize the under-explored one
...
def _parse_decision(raw: str) -> dict:
    try:
        ...
        if "underexplored_entities" not in parsed:
...
def _to_decision(parsed: dict) -> Decision:
    ...
    # Phase 1 safety: if comparison detected and coverage unbalanced, force insufficient
    is_comparison = parsed.get("is_comparison_query", False)
    underexplored = parsed.get("underexplored_entities", [])
    balance = parsed.get("entity_coverage_balance", 1.0)
    
    sufficient = parsed.get("sufficient", False)
    if is_comparison and underexplored and balance < 0.7:
        # Force insufficient if we detected a comparison with unbalanced coverage
        sufficient = False
        parsed["reason"] = f"Comparison but unbalanced: {balance:.1f}. Under: {underexplored}"
