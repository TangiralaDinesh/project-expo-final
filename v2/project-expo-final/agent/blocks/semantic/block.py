...
async def _run_critique_on_retrieval(
    ...
    *,
    ...
    """
    Run multi-persona critique on the retrieved chunks to identify gaps.
    ...
    try:
        ...
        artifact = "\n\n".join([
            f"[{c.source_url or 'internal'}] {c.text[:300]}"
            for c in top_chunks[:5]  # Limit to top 5 for critique
        ])
        ...
        # Extract gaps from disagreements
        gap_queries = []
        ...
        # Heuristic: if a concern mentions missing entity or incomplete comparison, flag it
        missing_patterns = ["missing", "incomplete", "without", "lacks", "not covered", "overlooked"]
        for concern in all_concerns:
            concern_lower = concern.lower()
            if any(pattern in concern_lower for pattern in missing_patterns):
                gap_queries.append(f"Explore missing aspect: {concern[:100]}")
                logger.info(f"Critique identified gap: {concern[:100]}")
