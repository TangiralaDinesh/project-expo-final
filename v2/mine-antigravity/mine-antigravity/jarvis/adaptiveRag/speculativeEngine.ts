...
export async function * speculativeStream(
  ...
  // ── Phase 3: Wait for retrieval (respect retrieval timeout, no artificial cap) ────────
  // Retrieval may already be done by the time streaming finishes.
  // Wait up to retrieval timeout to get actual context (don't artificially cap at 100ms).
  if (!retrievalDone) {
    await Promise.race([
      retrievalRace,
      new Promise<void>(r => setTimeout(r, 800)),  // Full ~800ms window for retrieval
    ])
  }
